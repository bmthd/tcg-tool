...
const isRefObject = <T extends object>(
	arg: RefObject<T> | T,
): arg is RefObject<T> => Object.hasOwn(arg, "current");
...
>(
	...
	useLayoutEffect(() => {
		const targetElement = isRefObject(target)
			? (target.current as T)
			: (target as T);
...
>(
	...
	handlerFactory: (
		node: TargetElement,
	) => (event: HTMLElementEventMap[EventName]) => unknown,
	options?: Omit<AddEventListenerOptions, "signal">,
