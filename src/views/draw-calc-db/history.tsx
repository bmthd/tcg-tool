							...
							<div className="flex-1">
								...
								<div className="flex items-center gap-2 mb-3">
									...
									<Text className="font-medium text-slate-200">
										{gameTemplates[calc.gameTemplate]?.name ||
											calc.gameTemplate}{" "}
										...
										{calc.targetCards?.map((card) => (
											...
											>
												...
												<span className="text-slate-400">
													(デッキ{card.K_in_deck}枚中{card.k_desired}枚引きたい)
												</span>
											</div>
										))}
									</div>
									...
									<div className="grid grid-cols-2 gap-4 mb-3">
										<div className="bg-slate-700/50 rounded p-3 text-center">
											...
											<Text className="font-bold text-sky-100">
												{(calc.result.probExactly * 100).toFixed(2)}%
											</Text>
										</div>
										<div className="bg-slate-700/50 rounded p-3 text-center">
											...
											<Text className="font-bold text-green-100">
												{(calc.result.probAtLeast * 100).toFixed(2)}%
											</Text>
										</div>
									</div>
								)}
